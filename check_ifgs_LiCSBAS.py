...
ifgs_to_check = "./to_check_in_order_of_nans.txt"#"./bad_ifg.txt"
...
if jasmin:
    ...
    to_exclude_savefile = f'./{frame_name}_to_exclude.txt'
...
if ifgs_to_check:    
    with open(ifgs_to_check) as f:
        ifgdates = [line.strip() for line in f]
else:
...
def update_plot(frame):
    ...
    if paused:
        return
    if review_mode:
        if current_index == 0 or review_index == 0:
            show_review_image()
        return
...
def exclude_ifg(event):
    ...
    if review_index < len(bad_ifg):
        ifgd = bad_ifg[review_index]
        with open(to_exclude_savefile, 'a') as file:
            print("opening exclude file")
            file.write(f"{ifgd}\n")
            print("wrote to file")
        review_index += 1
        show_review_image()
...
if jasmin:
    ...
    btn_excl.on_clicked(exclude_ifg)
